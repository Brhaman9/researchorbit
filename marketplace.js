...
(function () {
  ...
  // Storage Keys (Clean v7 storage)
  const STORAGE_TASKS = 'researchorbit_tasks_v7';
  const STORAGE_USERS = 'researchorbit_users_v7';
  const STORAGE_ACTIVE_USER = 'researchorbit_active_user_v7';
  const STORAGE_NOTIFICATIONS = 'researchorbit_notifications_v7';
  const STORAGE_CATEGORIES = 'researchorbit_categories_v7';
  ...
  const SEED_USERS = [
    ...
    },
    {
      ...
      institution: 'Division of Medical Research & Epidemiology',
  ...
  const SEED_TASKS = [
    {
      ...
      requirements: {
        ...
        expected_format: 'MS Word format strictly adhering to University Dissertation Guidelines',
  ...
  class MarketplaceUI {
    ...
    initElements() {
      ...
      this.dashboardModal = document.getElementById('modal-dashboard');
    }
    ...
    bindEvents() {
      ...
      if (dashboardBtn) {
        dashboardBtn.addEventListener('click', () => this.openDashboardModal('posted'));
      }
    ...
    openTaskDetailsModal(taskId) {
      ...
      if (creatorEl) {
        creatorEl.innerHTML = `
          ...
          <div>
            <strong>${creator.name}</strong>
            <small>${creator.designation} (${creator.institution || 'Medical College'})</small>
          </div>
        `;
      }
    ...
    // ==========================================
    // 5. User Dashboard ("My Freelancing")
    // ==========================================
    openDashboardModal(defaultTab = 'posted') {
      ...
      const renderTab = (tabName) => {
        ...
        let filtered = [];
        if (tabName === 'posted') {
          filtered = allTasks.filter((t) => t.creator_id === activeUser.id);
        } else if (tabName === 'working') {
          filtered = allTasks.filter((t) => t.assigned_user_id === activeUser.id && t.status !== 'Completed');
        } else if (tabName === 'completed') {
          filtered = allTasks.filter(
            (t) => (t.creator_id === activeUser.id || t.assigned_user_id === activeUser.id) && t.status === 'Completed'
          );
        } else if (tabName === 'revisions') {
          filtered = allTasks.filter(
            (t) => (t.creator_id === activeUser.id || t.assigned_user_id === activeUser.id) && t.status === 'Revision Requested'
          );
        }

        if (filtered.length === 0) {
          contentEl.innerHTML = `<div class="notif-empty">No tasks in this category.</div>`;
          return;
        }

        contentEl.innerHTML = filtered
          .map((task) => {
            const statusClass = task.status.toLowerCase().replace(/\s+/g, '-');
            return `
            <div class="dash-task-row">
              <div class="dash-task-main">
                ...
                <span class="task-status-badge status-${statusClass}">${task.status}</span>
                <h4>${task.title}</h4>
                <small>Deadline: ${formatDate(task.deadline)} • Created: ${formatDate(task.created_at)}</small>
              </div>
              <button class="button button-secondary btn-dash-view" data-id="${task.id}" type="button">
                Open Task
              </button>
            </div>
          `;
        ...
        contentEl.querySelectorAll('.btn-dash-view').forEach((b) => {
          b.addEventListener('click', (e) => {
            this.dashboardModal.classList.remove('open');
            this.openTaskDetailsModal(e.target.getAttribute('data-id'));
          });
        });
      };
      ...
      this.dashboardModal.classList.add('open');
    }
  }
