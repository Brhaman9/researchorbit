...
// Freelancing project category filter & prefill
...
const hireButtons = document.querySelectorAll(".project-btn");
if (hireButtons.length > 0 && projectTitleInput) {
  hireButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const title = btn.getAttribute("data-project-title");
      if (title) {
        projectTitleInput.value = `Inquiry: ${title}`;
      }
    });
  });
}
